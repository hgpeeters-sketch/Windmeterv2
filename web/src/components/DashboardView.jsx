...
// Roll tile: big number + PORT/STBD/LEVEL
function RollTile({ roll, onRequestPerms }) {
  const containerRef = useRef(null)
  const [fontSize, setFontSize] = useState(80)
  ...
  return (
    <div ref={containerRef} style={{ flex: 3, minHeight: 0, background: BG(2), position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', paddingLeft: 6 }}>
        {roll !== null
          ? <span style={{ fontSize, fontWeight: 900, fontFamily: 'monospace', color: FG(2), lineHeight: 1, letterSpacing: '-0.02em' }}>{text}</span>
          : <button onClick={onRequestPerms} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.25)', color: 'rgba(255,255,255,0.6)', fontFamily: 'monospace', fontSize: 12, padding: '10px 18px', cursor: 'pointer', letterSpacing: '0.1em' }}>
              ENABLE MOTION
            </button>
        }
      </div>
...
export default function DashboardView() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      {/* Roll */}
      <RollTile roll={roll} onRequestPerms={requestOrientation} />
